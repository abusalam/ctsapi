    ...
    public class ChequeReceivedService : IChequeReceivedService
    {
        ...
        public async Task<Int16?> ChequeReceived(ChequeReceivedDTO chequeReceivedDTO)
        {
            ...
            foreach (var chequeReceivedDamagedDetail in chequeReceivedDTO.ChequeReceivedDamagedDetails)
            {
                ...
                var invoiceDetails = await _ChequeInvoiceDetailRepository.GetSingleSelectedColumnByConditionAsync(entity => entity.Id == invoiceDeatilsId, entity => new
                {
                    Start = entity.Start,
                    End = entity.End
                });
                ...
                {
                    chequeReceivedModel.Add(new ChequeReceivedModel { Start = item.start, End = item.end, Quantity = (item.end - item.start) + 1, InvoiceId= chequeReceivedDTO.InvoiceId, InvoiceDeatilsId = invoiceDeatilsId, Status = chequeReceivedDTO.Status, TreasuryCode = chequeReceivedDTO.TreasuryCode, MicrCode = chequeReceivedDTO.MicrCode, Series = chequeReceivedDTO.Series });
                }
                //
