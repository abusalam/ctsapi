...
namespace CTS_BE.BAL.Services.Pension
{
    public abstract class BaseService : IBaseService
    {
        ...
        protected void SetPreparer<T>(T entity)
            where T : class
        {
            entity?.GetType()?.GetProperty("PreparedBy")?.SetValue(entity, _userId);
            entity
                ?.GetType()
                ?.GetProperty("PreparedOn")
                ?.SetValue(entity, DateOnly.FromDateTime(DateTime.Now));
        }

        protected void SetUpdatedBy<T>(T entity)
            where T : class
        {
