...
namespace CTS_BE.Controllers.Pension
{
    ...
    public class NomineeController : ApiBaseController
    {
        ...
        [Tags("Pension: Nominee Details")]
        [OpenApi]
        public async Task<JsonAPIResponse<NomineeListResponseDTO>> GetNomineesByPpoId(
            int ppoId
        )
        {
            JsonAPIResponse<NomineeListResponseDTO> response = new(){
                ApiResponseStatus = Enum.APIResponseStatus.Success,
                Message = $"PPO Nominee Details received sucessfully!"
            };
            try {
                response.Result = await _nomineeService.GetNomineeByPpoId(
                    ppoId,
                    GetTreasuryCode()
                );
            }
