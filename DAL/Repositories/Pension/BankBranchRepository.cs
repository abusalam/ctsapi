    ...
    public class BankBranchRepository : Repository<Branch, PensionDbContext>, IBankBranchRepository
    {
        ...
        public async Task<Bank?> GetBankById(string treasuryCode, long bankId)
        {
            return await _context
                .Banks.Where(entity => entity.ActiveFlag && entity.Id == bankId)
                .FirstOrDefaultAsync();
        }
        ...
        {
            return await _context
                .Branches.Where(entity => entity.ActiveFlag && entity.Id == branchId)
        ...
        {
            return await _context.Banks.Where(entity => entity.ActiveFlag).ToListAsync();
        }

        public async Task<List<Branch>> GetBranchesByBankId(string treasuryCode, long bankId)
        {
            return await _context
                .Branches.Where(entity => entity.ActiveFlag && entity.BankId == bankId)
                .Include(entity => entity.Bank)
                .ToListAsync();
        }
        ...
        public async Task<string> GetBankBranchName(string treasuryCode, long branchId)
        {
            var bankBranchName =
                await _context
                    .Branches.Where(entity => entity.Id == branchId)
                    .Include(entity => entity.Bank)
        ...
        public async Task<string> GetBankBranchNameByPpoId(string treasuryCode, int ppoId)
        {
            var bankBranchName =
                await _context
                    ...
                    .Select(entity =>
                        entity.Branch.Bank.BankName + " - " + entity.Branch.BranchName
                    )
